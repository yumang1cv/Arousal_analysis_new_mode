# -*- coding:utf-8 -*-
# @FileName  :Arousal_fre_line_radar_combine.py
...
"""
    ...
    Non-locomtion : 8-ImmobilityJIDE
...
def combine_data(data_name):
    # data = data_name
    data_name.columns = behavior_labels

    data_name['Locomotion'] = data_name.iloc[:, 0:1].sum(axis=1) + data_name.iloc[:, 1:2].sum(axis=1) + data_name.iloc[
                                                                                                        :, 3:4].sum(
        axis=1) + \
                              data_name.iloc[:, 10:11].sum(axis=1) + data_name.iloc[:, 12:13].sum(
        axis=1) + data_name.iloc[:, 13:14].sum(axis=1) + \
                              data_name.iloc[:, 15:16].sum(axis=1)
    data_name['Exploration'] = data_name.iloc[:, 2:3].sum(axis=1) + data_name.iloc[:, 5:6].sum(axis=1) + data_name.iloc[
                                                                                                         :, 6:7].sum(
        axis=1) + \
                               data_name.iloc[:, 9:10].sum(axis=1)
    data_name['Maintenance'] = data_name.iloc[:, 11:12].sum(axis=1)
    data_name['Non-locomtion'] = data_name.iloc[:, 7:8].sum(axis=1)
    data_name['Posture'] = data_name.iloc[:, 4:5].sum(axis=1) + data_name.iloc[:, 8:9].sum(axis=1) + data_name.iloc[:,
                                                                                                     14:15].sum(axis=1)

    data_name = data_name.drop(data_name.columns[0:16], axis=1)
...
if __name__ == '__main__':
    ...
    labels = ['Locomotion', 'Exploration', 'Maintenance', 'Non-locomtion', 'Posture']

    df1 = combine_data(df1)
    df2 = combine_data(df2)
    df3 = combine_data(df3)
    df4 = combine_data(df4)
    df5 = combine_data(df5)
